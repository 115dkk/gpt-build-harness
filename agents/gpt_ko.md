...
description: >-
  ...
  쓴다. 다른 effort는 쓰지 않는다. 이 릴레이는 **도구가 필요 없는 순수 추론
  전용**이다(붙여 준 코드·로그에 대한 리뷰·검증, 교차 검증, 2차 의견, 지식
  조회). GPT가 파일을 직접 뒤지고 고쳐야 하는 작업이면 이 릴레이 대신
  tools/gpt-agent.ps1(어느 세션에서나 자식 claude -p로 도구 쥔 Daybreak를
  띄움)이나 프록시 세션의 gpt-worker를 쓴다. Daybreak는 아주 좋은 백엔드
