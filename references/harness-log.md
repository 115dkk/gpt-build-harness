...
### 다음 (스킬 배포)
- 별도 repo(public MIT): 방법론(SKILL.md/references, setup은 민감부 제거판)만.
- 릴리즈 zip: 도구(gpt-relay/proxy/cc, agents) + 민감 setup + install 스크립트.
- 설치 파일 3종: Claude Code 전역(~/.claude), Codex 전역, Claude 앱(claude.ai zip).
