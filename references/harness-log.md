...
> **읽기 전에.** 세션 1의 모델·effort 지침("Luna xhigh/max가 기본, Luna low~medium은 아주 단순한 작업만, Terra 비권장")은 **측정 없이 인상으로 적은 것이었고, 세션 2의 실측에서 대부분 뒤집혔다.** 티어를 고를 때는 `tiers.md`를, 위임 프롬프트를 쓸 때는 `prompt-rules.md`를 정본으로 삼는다. 세션 1의 기록은 형태·오케스트레이션·함정 쪽에서 여전히 유효하다.
...
## 세션 2 (2026-07-25) — 실측 배터리

토이 세션에서 인상으로 적어둔 티어 지침을 신뢰할 수 없어, 정답이 확정된 과제로 통제 실험을 돌려 전부 데이터로 교체했다. 원 데이터와 실험 설계는 `measurements.md`, 결론은 `tiers.md`와 `prompt-rules.md`.

### 방법
- 호출은 `gpt` 서브에이전트를 거치지 않고 `gpt-relay.ps1`을 pwsh 7로 직접 실행했다. 서브에이전트 경로는 결과가 표시 계층을 거치며 오염되고 계측이 흐려진다.
- 채점은 인상이 아니라 **기계**로 했다. 검증 과제는 심은 결함의 위치·수정·트리거가 정답으로 확정되어 있고, 구현 과제는 숨긴 테스트 35개를 MSVC로 컴파일·실행해 채점했다.
- 보상해킹을 잡으려고 대조군을 넣었다. 결함이 없는 판(지어내는지), 존재하지 않는 함수(날조하는지), 실행 신고 줄(안 돌린 것을 돌렸다 하는지).

### 뒤집힌 것
1. **"Luna low~medium은 아주 단순한 작업만"은 틀렸다.** luna low가 정밀 스펙 구현에서 30초·1,625토큰에 35/35, `/W4` 경고 0을 냈다.
2. **"Luna xhigh/max가 기본"은 틀렸다.** xhigh는 medium과 같은 소견을 내며 시간·토큰을 9배 썼고, max는 같은 검증 과제를 2회 중 1회 놓쳤다. effort는 단조가 아니다.
3. **"Terra 비권장"은 근거가 없었다.** Terra medium이 검증 2/2를 13초·675토큰에 냈다. 측정 범위에서 가장 쌌다.
4. **검증 기본은 SOL이다.** 의미·순서 결함에서 Luna는 low·medium·max가 모두 놓쳤고 xhigh만 잡았다. SOL은 medium부터 전부 잡았다.

### 새로 확인한 것
5. **날조는 프롬프트의 성질이다.** 같은 luna low가 탈출구 없이는 없는 멤버 변수까지 창작했고, "확인 못 하면 못 한다고 하라" 한 문단을 넣자 정확히 거부했다. 품질은 오히려 올라갔다. SOL만 탈출구 없이도 버텼다.
6. **위임 실패의 원인은 실력이 아니라 문장이었다.** "헤더를 다시 쓰지 말라"를 문자 그대로 따라 절반이 타입 별칭을 빠뜨려 컴파일이 깨졌다. 별칭 한 줄만 넣으니 전부 통과했다. 문구를 완결 조건으로 바꾸자 그 실패가 사라졌다.
7. **HTML 엔티티는 표시 계층 현상이고 복원이 불필요하다.** Claude 에이전트 반환값도 똑같이 깨져 오며, `journal.jsonl`에는 원문이 그대로 있다. 저널에서 꺼낸 코드는 그대로 컴파일돼 통과했다. 종래의 "심기 전 복원 필수"는 저널에서 읽는 것으로 대체한다.
8. **프록시의 도구 호출 변환이 작동한다.** 도구 정의를 준 요청에 `stop_reason: "tool_use"`와 올바른 인자가 돌아왔다. 워커·메인 형태에서 GPT가 실제로 탐색한다.
9. **Workflow에서 `agentType: 'gpt'`가 정상 작동한다.** StructuredOutput 스키마도 채워 돌아왔다. 같은 검증 과제에서 GPT와 Claude 에이전트가 모두 적중했으므로, 그 난도에서는 Claude 토큰을 아끼는 쪽이 낫다.

### 새 함정
10. **릴레이 출력은 PowerShell `>` 로 못 잡는다.** `[Console]::Out.Write`가 콘솔 핸들에 직접 쓴다. 빈 파일만 남는다.
11. **cmd에서 인용 경로 끝 역슬래시가 닫는 따옴표를 먹는다.** `/Fo:"...\obj\"`가 명령줄을 통째로 깨뜨린다(cl D8003).

### 유지한 고정점 (사용자 방침)
- SOL은 high를 넘기지 않는다.
- Luna max는 계속 쓴다. 다만 위 분산 관측이 있으니 판정을 단독으로 맡기지 않는다.
