...
description: >-
  ...
  main loop model (auto-compact at 500k on Daybreak, 860k of a 1M window on
  ASTRA). Backend implementation goes to the
...
- **Effort: `high` by default, and high is the sweet spot.** The harness exposes three rungs, `high`, `xhigh` and `max`; `low` and `medium` are never used. On Daybreak Blue, `max` stays reserved for hopelessly hard problems. On ASTRA the two rungs above high are open but **unmeasured**, so climb them deliberately and report what you find rather than defaulting upward. `ultra` is not an effort this path can send: the responses endpoint answers HTTP 400 for it on both models, listing none/minimal/low/medium/high/xhigh/max (measured 2026-09-05). Ultra is a Codex client mode (maximum reasoning with automatic task delegation), so it is selectable for ASTRA in the Codex app and CLI but never in the harness, which calls the endpoint directly. The relay, the proxy, the launcher and the direct worker all default to daybreak-blue/high, so omitting the instruction still follows policy.
...
4. **Main loop model (the `gpt-cc.ps1` launcher)**: run all of Claude Code on GPT. The launcher sizes the window from `-Base`: Daybreak Blue **auto-compacts at 500k** (`CLAUDE_CODE_AUTO_COMPACT_WINDOW=500000`, the same knob as the `autoCompactWindow` setting), ASTRA runs a **1M window and compacts at 860k** (`CLAUDE_CODE_MAX_CONTEXT_TOKENS=1000000` as well). Both override the fallback window Claude Code uses for unrecognised model slugs.
