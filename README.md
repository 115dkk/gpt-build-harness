...
- **모델은 둘이다.** 백엔드 노동은 Daybreak Blue(`gpt-daybreak-blue`), 화면과 i18n과 가장 어려운 문제는 ASTRA(`gpt-6-astra`, GPT-6 Astra, 2026-09-03 공개, 슬러그 2026-09-05 실측 확인). effort는 `high`가 기본이자 스윗스팟이고 그 위로 `xhigh`와 `max`를 연다(Daybreak의 `max`는 절망적으로 어려운 문제 전용, ASTRA의 두 단은 아직 미측정). 컨텍스트는 Daybreak가 500k 압축, ASTRA가 1M 윈도우에 860k 압축이다.
...
- **단, 맨땅에서 세우는 프론트엔드는 ASTRA에 주지 않는다.** ASTRA는 골조가 선 프론트엔드를 고칠 때 쓸 만하다. 골조와 본이 되는 첫 화면은 Claude가 세우고, 그다음부터 ASTRA가 이어받는다. 설계는 어느 경우에도 ASTRA가 하지 않는다.
